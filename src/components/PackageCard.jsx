  ...
  return (
    <div className={`block relative bg-white ${compact ? 'rounded-2xl' : 'rounded-3xl'} overflow-hidden shadow-xs border border-neutral-200/90 hover:shadow-md hover:border-neutral-300 transition-all duration-300 group`}>
      ...
      <div className={`${compact ? 'p-3' : 'p-3.5 xs:p-4'} bg-neutral-50/60 border-t border-neutral-100 flex items-center justify-between gap-2`}>
        ...
        {/* Tombol CTA -> Buka Halaman Detail (Icon-only di mode compact) */}
        <Link
          href={`/paket/${packageSlug}`}
          aria-label={`Lihat detail paket ${name}`}
          className={`inline-flex items-center justify-center bg-neutral-900 hover:bg-neutral-800 text-white font-bold ${
            compact
              ? 'w-8 h-8 rounded-xl'
              : 'text-xs p-2.5 xs:px-4 xs:py-2.5 rounded-xl gap-1.5'
          } transition-all shadow-xs hover:-translate-y-0.5 active:scale-95 shrink-0`}
        >
          {!compact && <span>Lihat Detail</span>}
          <svg 
            className={`${compact ? 'w-4 h-4' : 'w-3.5 h-3.5'} group-hover:translate-x-0.5 transition-transform duration-200`} 
            fill="none" 
            viewBox="0 0 24 24" 
            stroke="currentColor"
          >
